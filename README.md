...
<details open>
<summary>Gemini API 종속성 안내</summary>
...
도구를 붙이기 전에 `CLAUDE.md`로 제약을 먼저 걸었습니다. 규칙 없이 맡기면 베이스 클래스와 레지스트리 패턴이 생겨나고, 그 순간 설명할 수 없는 코드가 됩니다.
...
마지막 두 줄은 실제로 겪은 문제를 규칙으로 만든 것입니다. 전체 덮어쓰기가 파일을 망가뜨린 적이 있었고, 대체한 옛 코드가 지워지지 않고 아래에 새 코드만 추가되는 일이 반복됐습니다.

</details>


<details>
<summary>프롬프트 설계</summary>
...
브리프의 선택 항목이 있을 때와 없을 때를 프롬프트가 직접 분기합니다. 판단 기준은 **추론 가능한 것은 추론하고, 사실 영역은 생략한다**입니다.

```
[선택 항목 처리]
- tone이 없으면 keywords와 target에서 톤을 추론해 진행한다. 중단하지 않는다.
- multilingual이 true가 아니거나 키가 없으면: en은 한글명의 음차 또는 직역으로 충분하다.
- multilingual이 true이면: en은 영어권 화자가 뜻을 바로 알아듣는 이름으로 만든다.
  실재하는 영단어 하나 또는 영단어 두 개의 조합만 쓴다. 라틴어·그리스어 어근 조어와
  영단어를 억지로 붙인 합성어는 쓰지 않는다. 한글명의 의미를 영어로 다시 표현한 것이어야 한다.
  meaning에 영문명이 한글명의 의미를 어떻게 옮겼는지도 한 문장 덧붙인다.
- competitors가 있으면 comparison에 비교를 쓴다. 경쟁사 이름의 어감, 조어 방식, 길이,
  언어 구성만 비교한다. 그 회사의 사업 내용이나 시장 지위는 추측하지 않는다.
- competitors가 없거나 비어 있으면 comparison 필드를 만들지 않는다.
- notes에 이 단계와 관련된 요청이 있으면 반영한다. 없거나 무관하면 무시한다.
```

경쟁사 부분에 제약을 건 이유가 있습니다. 모델은 유명 브랜드는 알지만 무명 브랜드는 모르고, **모른다고 말하는 대신 그럴듯한 포지셔닝을 지어냅니다.** 파이프라인 첫 단계의 환각은 뒤로 갈수록 증폭되기 때문에, 지식이 필요 없는 영역(이름 문자열의 어감·조어 방식)으로만 비교 범위를 한정했습니다.

## 로고 프롬프트의 제약

이미지 생성 모델은 글자를 거의 항상 깨뜨립니다. 그래서 로고 프롬프트를 만드는 단계에 강한 금지 규칙을 넣었습니다.
...
세 번 반복되는 부정 명령이 과해 보이지만, 실측 결과 이 조합에서 글자가 섞여 나온 적이 없습니다. hex를 프롬프트에 직접 박은 것도 효과가 컸습니다 — 팔레트와 로고 3장의 색이 실제로 일치합니다.
...
| 2 | 다국어(한/영) 네이밍 동시 생성 | 브리프 `multilingual: true`이면 `en`을 음차가 아닌 별도 영문 네이밍으로 생성 | [경쟁사](#경쟁사) · [비교](#비교) · [조건부 처리](#조건부-처리) |
...
`multilingual: true`와 `competitors`가 함께 들어간 브리프입니다. 영문 네이밍이 음차가 아니라 의미 중심 조어(`Prime Origin`, `Verity Guild`)로 바뀌고, 경쟁사와의 어감 차이도 함께 생성됩니다.
...
![다국어·경쟁사 분석](readImage/05brief_multilangual_korean.png)
